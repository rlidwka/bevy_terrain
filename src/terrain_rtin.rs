...
use crate::rtin::{
    BinId,
    Vec2u32,
    get_index_level_start,
    get_triangle_children_bin_ids,
    get_triangle_children_indices,
    get_triangle_coords,
    index_to_bin_id,
    pixel_coords_for_triangle_mid_point,
};
...
impl Default for RtinParams {
    fn default() -> Self {
        Self {
            error_threshold: 0.2,
            load_options: TerrainImageLoadOptions::default(),
        }
    }
}
...
pub fn rtin_load_terrain(filename: &str, rtin_params: &RtinParams) -> (Mesh, Mesh) {
    ...
    let shaded_mesh = rtin_make_terrain_mesh(&terrain_mesh_data, &rtin_params.load_options, false);
    let wireframe_mesh =
        rtin_make_terrain_mesh(&terrain_mesh_data, &rtin_params.load_options, true);
...
pub fn rtin_make_terrain_mesh(
    ...
    // let grad = noise::utils::ColorGradient::new().build_terrain_gradient();
    ...
    for vertex in &terrain_mesh_data.vertices {
        ...
        // let color = grad.get_color(vertex.y as f64 * 2. - 1.);
        let raw_float: Srgb<f32> = Srgb::<f32>::from_linear(color.into());
        colors.push([raw_float.red, raw_float.green, raw_float.blue]);
        // colors.push([color[0] as f32 / 255.0, color[1] as f32 / 255.0, color[2] as f32 / 255.0]);
        uvs.push([vertex.x / 100., vertex.z / 100.]);
        // uvs.push([vertex.y, vertex.y]);
    }
    ...
    mesh.insert_attribute(TerrainMaterial::ATTRIBUTE_COLOR, colors);
...
// impl<T> VecClamp for nalgebra::Vector2<T>
// where
//     T: Scalar + Ord + Copy,
// {
//     fn clamp(&self, left: &Self, right: &Self) -> Self {
...
pub fn sample_heightmap_height_corner_mean(heightmap: &HeightMapU16, corner_u32: Vec2u32) -> f32 {
    ...
    heightmap.get_pixel(new_corner[0], new_corner[1]).0[0] as f32 / u16::MAX as f32
}
